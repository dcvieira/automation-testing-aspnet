...
namespace CarvedRock.InnerLoop.WebApp.Test;
  ...
  : IClassFixture<CustomWebAppFactory>
{
    ...
    public async Task PostLoggedInCheckoutPageWithItems()
    {
        ...
        {
            var emailResult = await emailClient.GetFromJsonAsync<EmailResult>("/api/messages");
            var emailList = emailResult!.results;
            var email = emailList!.FirstOrDefault(e => e.To[0] == "erik@test.com");
    ...
    public record EmailResult(IEnumerable<EmailModel> results);
