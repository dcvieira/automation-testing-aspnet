...
public class CustomWebAppFactory(SharedFixture fixture) : WebApplicationFactory<Program>
{
    ...
    {
        var productService = services.SingleOrDefault(d => d.ServiceType == typeof(IProductService));
        services.Remove(productService!);

        var productServiceMock = Substitute.For<IProductService>();

        productServiceMock.GetProductsAsync(Arg.Any<string>()).Returns(args => SharedFixture.OriginalProducts
